...
# utils/seasonal_albedo.py

import ee
from datetime import datetime, date
from city_metrix.metrix_model import Layer, GeoExtent

# reuse CLEAR_THRESHOLD and seasonal_s2_albedo from your script

class SeasonalAlbedo(Layer):
    OUTPUT_FILE_FORMAT = ".tif"
    MAJOR_NAMING_ATTS = ["season_start", "season_end", "hemisphere"]
    MINOR_NAMING_ATTS = None
    PROCESSING_TILE_SIDE_M = 5000  # adjust if you need to tile exports

    def get_data(self, bbox: GeoExtent, spatial_resolution=10, resampling_method=None):
        """
        Return a Sentinel‑2 seasonal albedo median composite as an ee.Image.
        bbox: a GeoExtent defining the area of interest.
        spatial_resolution: defaults to 10 m (S2 resolution).
        """
        if resampling_method is not None:
            raise Exception("resampling_method cannot be specified.")

        # Convert GeoExtent or shapely bbox to EE rectangle in WGS84
        if hasattr(bbox, "bbox"):
            minx, miny, maxx, maxy = bbox.bbox
        else:
            minx, miny, maxx, maxy = bbox.bounds
        aoi = ee.Geometry.Rectangle([float(minx), float(miny), float(maxx), float(maxy)],
                                    proj="EPSG:4326", geodesic=False)

        # Use your helper to build the albedo composite
        img = seasonal_s2_albedo(aoi)

        # Set a default projection so downloads are consistent
